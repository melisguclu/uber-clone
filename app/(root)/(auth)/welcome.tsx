...
const Onboarding = () => {
  ...
  return(
    <SafeAreaView edges={['bottom']} className='flex h-full items-center justify-between bg-white'>
      <TouchableOpacity
        onPress={() => {
      ...
      >
        {
          onboarding.map((item) => (
            <View key={item.id} className='flex p-5 items-center justify-center'>
              ...
              <Text className='text-md font-JakartaSemiBold mx-10 mt-3 text-center text-[#858585]'>{item.description}</Text>
            </View>
          ))
        }
      </Swiper>
